...
SLQ_KRYLOV_DIMENSION = 150
SLQ_RANDOM_VECTORS = 50
NEUMANN_TERMS = 25
HUTCHINSON_RANDOM_VECTORS = 50

def neumann_solve_cupy(W_cp, Z, lam):
    """
    Finds (I - lam*W)^-1 Z using the Neumann on the GPU.
    """
    res = cp.copy(Z)
    current_term = cp.copy(Z)

    for _ in range(NEUMANN_TERMS):
        current_term = lam * W_cp.dot(current_term)
        res += current_term

    return res

def batched_lanczos_logdet_cupy(A):
    """
    Finds log(det(A)) using SLQ
    """
    n = A.shape[0]

    def M_dot(V):
        return A.T.dot(A.dot(V))

    # Rademacher vectors
    V = cp.random.choice(cp.array([-1.0, 1.0]), size=(n, SLQ_RANDOM_VECTORS))
    V_norms = cp.linalg.norm(V, axis=0)
    Q = V / V_norms

    alphas = cp.zeros((SLQ_KRYLOV_DIMENSION, SLQ_RANDOM_VECTORS))
    betas = cp.zeros((SLQ_KRYLOV_DIMENSION, SLQ_RANDOM_VECTORS))
    Q_prev = cp.zeros((n, SLQ_RANDOM_VECTORS))
    beta = cp.zeros(SLQ_RANDOM_VECTORS)

    # Batched lanczos iterations
    for j in range(SLQ_KRYLOV_DIMENSION):
        Z = M_dot(Q)

        alpha = cp.sum(Q * Z, axis=0)
        alphas[j, :] = alpha

        Z = Z - Q * alpha - Q_prev * beta
        beta = cp.linalg.norm(Z, axis=0)

        if j < SLQ_KRYLOV_DIMENSION - 1:
            betas[j, :] = beta

        Q_prev = Q

        # division by zero check
        valid_mask = beta > 1e-12
        Q = cp.zeros_like(Z)
        Q[:, valid_mask] = Z[:, valid_mask] / beta[valid_mask]

    T_m = cp.zeros((SLQ_RANDOM_VECTORS, SLQ_KRYLOV_DIMENSION, SLQ_KRYLOV_DIMENSION))

    idx = cp.arange(SLQ_KRYLOV_DIMENSION)
    T_m[:, idx, idx] = alphas.T

    idx_off = cp.arange(SLQ_KRYLOV_DIMENSION - 1)
    T_m[:, idx_off, idx_off + 1] = betas[:-1, :].T
    T_m[:, idx_off + 1, idx_off] = betas[:-1, :].T

    # batched eigendecomposition
    eigvals, eigvecs = cp.linalg.eigh(T_m)

    tau = eigvecs[:, 0, :] ** 2

    valid_eigs_mask = eigvals > 1e-12
    safe_eigvals = cp.where(valid_eigs_mask, eigvals, 1.0)

    log_eigs = cp.log(safe_eigvals)
    log_eigs = log_eigs * valid_eigs_mask

    quadratures = (V_norms ** 2) * cp.sum(tau * log_eigs, axis=1)
    log_det_estimate = cp.sum(quadratures)

    return 0.5 * (log_det_estimate.item() / SLQ_RANDOM_VECTORS)

def err_c_loglik_sp_cupy_slq(lam, n, y, ylag, x, xlag, I, Wsp):
    """
    Evaluates the Spatial Error Model log-likelihood.
    Requires y, ylag, x, xlag, I, and Wsp to already be CuPy objects.
    """
    if isinstance(lam, (cp.ndarray, np.ndarray)):
        lam = float(lam.item())
    ...
    ee = cp.dot(e.T, e)
    sig2 = float(ee.item()) / n

    # 3. Log-Determinant (Jacobian), SLQ
    A = I - lam * Wsp
    try:
        jacob = batched_lanczos_logdet_cupy(A)
    except Exception:
        return float('inf')
...
class BaseML_Error(RegressionPropsY, RegressionPropsVM, REGI.Regimes_Frame):
    ...
    def __init__(self, y, x, w, method="full", epsilon=0.0000001, regimes_att=None):
        ...
        if methodML in ["FULL", "LU", "ORD", "GPU", "GPU-SLQ"]:
            if methodML == "FULL":
            ...
            elif methodML in ("GPU", "GPU-SLQ"):
                y_cp = cp.array(self.y)
                ylag_cp = cp.array(ylag)
                x_cp = cp.array(self.x)
                xlag_cp = cp.array(xlag)

                I_cp = cpsp.identity(w.n, format='csc')
                Wsp_cp = cpsp.csc_matrix(w.sparse)
                res = minimize_scalar(
                    err_c_loglik_sp_cupy_slq,
                    0.0,
                    bounds=(-1.0, 1.0),
        ...
        if methodML in ("GPU", "GPU-SLQ"):
            n = W.shape[0]

            if not cpsp.isspmatrix(W):
                W_cp = cpsp.csr_matrix(W)
            else:
                W_cp = W

            V = cp.random.choice(
                cp.array([-1.0, 1.0]),
                size=(n, HUTCHINSON_RANDOM_VECTORS)
            )

            # series expansion
            X = neumann_solve_cupy(W_cp, V, self.lam)
            WX = W_cp.dot(X)

            Y = neumann_solve_cupy(W_cp, WX, self.lam)
            WY = W_cp.dot(Y)

            # traces, batched dot products
            tr1 = float(cp.mean(cp.sum(V * WX, axis=0)).item())
            tr2 = float(cp.mean(cp.sum(V * WY, axis=0)).item())
            tr3 = float(cp.mean(cp.sum(WX * WX, axis=0)).item())
